...
modbus_package* write_read_modbus(uint16_t length, struct netconn *conn,modbus_package *query){
...
resp_0x05_0x06* request_0x05_0x06(modbus_package *query,uint16_t address,uint16_t count,struct netconn *conn){
	...
	uint16_t length = fill_req_0x05_0x06((req_0x05_0x06*) query->data, address, count);
	query->length = length+2;
	req_change_endian(query);
	modbus_package *answer  = write_read_modbus(length+2+6, conn, query);
		if (answer==NULL)
